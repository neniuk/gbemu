...
void Joypad::set_key(SDL_Scancode sc, bool pressed) {
    switch (sc) {
    case SDL_SCANCODE_RIGHT: keys_.right = pressed; break;
    case SDL_SCANCODE_LEFT: keys_.left = pressed; break;
    case SDL_SCANCODE_UP: keys_.up = pressed; break;
    case SDL_SCANCODE_DOWN: keys_.down = pressed; break;
    case SDL_SCANCODE_X: keys_.a = pressed; break;          // A
    case SDL_SCANCODE_Z: keys_.b = pressed; break;          // B
    case SDL_SCANCODE_RETURN: keys_.start = pressed; break; // Start
    case SDL_SCANCODE_RSHIFT: keys_.select = pressed; break;// Select
    default: break;
    }
}
...
uint8_t Joypad::compose_joyp() const {
    return static_cast<uint8_t>(0xC0 | select_bits_ | compute_low_nibble());
}

uint8_t Joypad::get_joyp() const {
    return compose_joyp();
}
